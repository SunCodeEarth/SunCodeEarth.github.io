...
class BubbleUI {
    ...
    */
     ...
     */
    subBubblePos (i, cx, cy, r, n, br = 0.88) {
        // shrink the R so that bubbles will not overlap
        let maxR = Math.min(r*br*0.5, br*2*Math.PI*1.5*r/2/n);
        return [cx+Math.cos(2*Math.PI*i/(n+1)), cy + Math.sin(2*Math.PI*i/(n+1))];
    }
    ...
    resize() {
        ...
        this.initR = (w - lm - rm) / 4 / this.colNum;
    ...
    reset() {
        ...
        for (var iB = 0; iB < this.nTop; iB++) {
            let xy = this.subBubblePos(iB, centerXs[iB], centerYs[iB], oR, this.maxSubN, 0.88);
